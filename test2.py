        ...
        with paddle.no_grad():
            ...
            for num, (image, mask, (H, W), maskpath) in enumerate(self.loader):
                ...
                for num in range(len(H)):
                    ...
                    mae_pred = F.interpolate(mae_pred, size=mae_mask.shape[2:], mode='bilinear', align_corners=False)
...
def select_best_model(model, model_list, save=None):
    ...
    for model_path in model_list:
        ...
        print(results)
        print('\n')
        all_res[model_path.split('/')[-1]] = results
    name = model_list[0].split('/')[-2]
    np.save(str(name) + '.npy', all_res)
...
if __name__=='__main__':
    ...
    model_list1 = [
#        "weight2/model-69.pdparams"
        "weightSwin2/model-64.pdparams"
    ]
    # select_best_model(HRSODNet(), model_list1, save='HRSODNet_Maps')
    # mutil_test_score('HRSODNet_Maps', 'HRSODNet_Scores')
#    select_best_model(HRSODNet(), model_list1)#, save='SwinNet_Maps1')
   # mutil_test_score('SwinNet_Maps1', 'SwinNet_Scores1')

    select_best_model(HRSODNet(), model_list1, '/home/aistudio/SwinNet_weight')
    mutil_test_score('/home/aistudio/SwinNet_weight', '/home/aistudio/scores/SwinNet_weight')
